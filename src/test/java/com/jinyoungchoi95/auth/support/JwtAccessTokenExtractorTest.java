...
class JwtAccessTokenExtractorTest {
    ...
    void extractNothing_empty() {
        ...
        assertThat(JwtAccessTokenExtractor.extract(request)).isEmpty();
    }
    ...
    void extract() {
        when(request.getHeader(AUTHORIZATION_HEADER_TYPE)).thenReturn("Token jwt.token.here");

        assertThat(JwtAccessTokenExtractor.extract(request)).isEqualTo(Optional.of("jwt.token.here"));
    }
}
